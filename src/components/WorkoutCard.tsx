    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          <span className={cn("text-[9px] font-semibold uppercase tracking-wider px-1.5 py-0.5 rounded-full", diffBadge[w.difficulty])}>
            {w.difficulty[0]}
          </span>
        </div>
        <div className="mt-1 flex items-center gap-3 text-[11px] text-muted-foreground tabular-nums">
          <span className="flex items-center gap-1"><Clock className="size-3" />{w.duration}m</span>
          <span className="flex items-center gap-1"><Flame className="size-3" />{w.calories}</span>
          <span className="flex items-center gap-1 truncate"><Dumbbell className="size-3" />{w.targetMuscles.slice(0, 2).join(" · ")}</span>
        </div>
      </div>
      ...
      >
        ...
        <div className="absolute inset-x-0 bottom-0 p-3">
          ...
          <div className="mt-1.5 flex items-center gap-1.5">
            <Pill icon={Clock} small>{w.duration}m</Pill>
            <Pill icon={Flame} small>{w.calories}</Pill>
          </div>
        </div>
      </div>
    </Link>
...
function Pill({ icon: Icon, children, small }: { icon: typeof Clock; children: React.ReactNode; small?: boolean }) {
  return (
    <span className={cn(
